...
def v4_get_callbacks_cleverstopper(log_dir, acc_threshhold, patience, dump_output, outputs_dir, dump_every, save_model_dir,
                                   acc_stopper_delayed, minimal_epoch, drop_percentage, clever_stop_suffix, is_distributed: bool = False) -> List[Callback]:
    # print('clever early stopping')
    callbacks = [
        ConsoleLogSaver(as_json=True, print_train_loss=True, save2file=True, log_dir=log_dir, dump_output=dump_output, outputs_dir=outputs_dir, dump_every=dump_every),
        BestStatsTracker(),
        CleverStopper(threshold=acc_threshhold, patience=patience, prev_acc_list_len=acc_stopper_delayed,
                      minimal_epoch=minimal_epoch, drop_percentage=drop_percentage, save_model_dir=save_model_dir,
                      outputs_dir=outputs_dir, clever_stop_suffix=clever_stop_suffix),
        GeneralModelSaver(save_model_dir),
    ]
    return callbacks


def v5_get_callbacks(log_dir, outputs_dir, save_model_dir, using_earlystop, acc_threshhold, patience, dump_output, dump_every,
                                   acc_stopper_delayed, minimal_epoch, drop_percentage, clever_stop_suffix, is_distributed: bool = False) -> List[Callback]:
    # print('clever early stopping')

    callbacks = [
        ConsoleLogSaver(as_json=True, print_train_loss=True, save2file=True, log_dir=log_dir, dump_output=dump_output,
                        outputs_dir=outputs_dir, dump_every=dump_every),
        BestStatsTracker(),
        # EarlyStopping_NoImprovement(threshold=acc_threshhold, patience=patience),
        GeneralModelSaver(save_model_dir),
    ]
    if using_earlystop:
        callbacks.insert(2, CleverStopper(threshold=acc_threshhold, patience=patience, prev_acc_list_len=acc_stopper_delayed,
                      minimal_epoch=minimal_epoch, drop_percentage=drop_percentage, save_model_dir=save_model_dir,
                      outputs_dir=outputs_dir, clever_stop_suffix=clever_stop_suffix))
...
class BestStatsTracker(Callback):
    def __init__(self):
        super(BestStatsTracker, self).__init__()
        self.callback_name = 'BestStatsTracker'
...
class ConsoleLogSaver(ConsoleLogger):
    def __init__(self, print_train_loss=False, as_json=False, save2file=True, log_dir=None, dump_output=True, outputs_dir=None, dump_every=50):
        ...
        self.aggregate_log = []
        self.callback_name = 'ConsoleLogSaver'
...
class GeneralModelSaver(Callback):
    ...
    def __init__(self, save_model_dir=None, only_saving_last=True):
        super(GeneralModelSaver, self).__init__()
        self.callback_name = 'GeneralModelSaver'
        self.save_model_dir = save_model_dir
        self.saving_mode = 'all'
        # ['all', 'only_last', 'None']
    ...
    def save_model(self, epoch=""):
        ...
        if model.name == 'Spk_Lst':
            enc_name = f"{model.name}_{epoch if epoch else 'final'}_spk_enc.pt"
            torch.save(model.speaker.encoder.state_dict(), os.path.join(self.save_model_dir, enc_name))
            dec_name = f"{model.name}_{epoch if epoch else 'final'}_spk_dec.pt"
            torch.save(model.speaker.decoder.state_dict(), os.path.join(self.save_model_dir, dec_name))

            enc_name = f"{model.name}_{epoch if epoch else 'final'}_lst_enc.pt"
            torch.save(model.listener.encoder.state_dict(), os.path.join(self.save_model_dir, enc_name))
            dec_name = f"{model.name}_{epoch if epoch else 'final'}_lst_dec.pt"
            torch.save(model.listener.decoder.state_dict(), os.path.join(self.save_model_dir, dec_name))

        else:
            enc_name = f"{model.name}_{epoch if epoch else 'final'}_enc.pt"
            torch.save(model.encoder.state_dict(), os.path.join(self.save_model_dir, enc_name))
            dec_name = f"{model.name}_{epoch if epoch else 'final'}_dec.pt"
            torch.save(model.decoder.state_dict(), os.path.join(self.save_model_dir, dec_name))

    def on_train_end(self):
        if self.saving_mode != 'None':
            self.save_model()

    def on_epoch_end(self, loss: float, _logs: Interaction, epoch: int):
        if self.saving_mode == 'every':
            self.save_model(epoch=epoch)

    def set_saving_mode(self, mode='all'):
...
class CleverStopper(EarlyStopper):
    def __init__(self,
                 threshold: float, field_name: str = "acc", validation: bool = True,
                 min_delta=1e-5,
                 patience=5, prev_acc_list_len=5, minimal_epoch=10, drop_percentage = 0.05, save_model_dir='',
                 outputs_dir='', clever_stop_suffix=''):
        super(CleverStopper, self).__init__(validation)
        self.threshold = threshold
        self.field_name = field_name
        # if "multi_acc": early_stopping use multi_acc as criterion, differ only for speaker
        # if "acc": early_stopping use normal acc
        ...
        self.prev_acc_list = []
        self.prev_acc_list_len = prev_acc_list_len
        self.minimal_epoch = minimal_epoch
        self.drop_percentage = drop_percentage

        self.clever_buffer = CleverBuffer(buffer_len=self.prev_acc_list_len, save_model_dir=save_model_dir, outputs_dir=outputs_dir, suffix=clever_stop_suffix)
        self.stopped_at_epoch = -1
    ...
    def on_validation_end(self, loss: float, logs: Interaction, epoch: int) -> None:
        ...
        self.trainer.should_stop, clever_stopped_epoch = self.should_stop(epoch, logs)
        if self.trainer.should_stop:
            self.clever_buffer.clever_save_model(clever_stopped_epoch)

    def should_stop(self, epoch, testlog) -> bool:
        if self.validation:
            assert (
                self.validation_stats
            ), "Validation data must be provided for early stooping to work"
            ...
            print('!!!!!!!!!!!!!!!! early_stop_called_when_its_not_validation !!!!!!!!!!!!!!!!')
            assert (
                self.train_stats
            ), "Training data must be provided for early stooping to work"
            loss, last_epoch_interactions = self.train_stats[-1]
        ...
        acc_collapse_stop = False

        current_acc = metric_mean
        # metric_mean: normal acc
        clever_stopped_epoch = None

        if len(self.prev_acc_list) < self.prev_acc_list_len:
            self.prev_acc_list.append(current_acc)
        else:
            if epoch < self.minimal_epoch:
                self.prev_acc_list.pop(0)
                self.prev_acc_list.append(current_acc)
            else:
                if current_acc >= self.prev_acc_list[0] * (1-self.drop_percentage):
                    self.wait = 0  # reset
                    acc_collapse_stop = False
                else:
                    self.wait += 1
                    if self.wait >= self.patience:
                        # self.stopped_epoch = epoch + 1
                        acc_collapse_stop = True
                        clever_stopped_epoch = epoch - (self.prev_acc_list_len - np.argmax(self.prev_acc_list))
                        self.stopped_at_epoch = clever_stopped_epoch

                self.prev_acc_list.pop(0)
                self.prev_acc_list.append(current_acc)

        self.clever_buffer.fill_buffer(self.trainer.game.model, testlog, epoch, acc_collapse_stop)

        # stop = acc_stop or acc_collapse_stop
        stop = acc_collapse_stop

        if stop:
            print(f'validation: {self.validation}')
            print(f'acc_collapse_stop:{acc_collapse_stop}')
            print('prev_accs:}')
            print(self.prev_acc_list)

        return stop, clever_stopped_epoch


class CleverBuffer(object):

    def __init__(self, buffer_len, save_model_dir, outputs_dir, suffix):
        self.model_buffer = []
        self.test_log_buffer = []
        self.epoch_buffer = []
        self.epoch_buffer_len = buffer_len
        self.save_model_dir = save_model_dir
        self.outputs_dir = outputs_dir
        self.suffix = suffix

    def fill_buffer(self, model, testlog, epoch, acc_collapse_stop=False):
        if acc_collapse_stop:
            return
        else:
            if len(self.model_buffer) < self.epoch_buffer_len:
                self.model_buffer.append(model)
                self.test_log_buffer.append(testlog)
                self.epoch_buffer.append(epoch)
            else:
                self.model_buffer.pop(0)
                self.model_buffer.append(model)
                self.test_log_buffer.pop(0)
                self.test_log_buffer.append(testlog)
                self.epoch_buffer.pop(0)
                self.epoch_buffer.append(epoch)

    def clever_save_model(self, epoch):
        clever_stopper_label = 'cleverstop'+self.suffix
        idx = self.epoch_buffer.index(epoch)
        model = self.model_buffer[idx]
        test_logs = self.test_log_buffer[idx]

        model_name = f"{model.name}_{epoch}_{clever_stopper_label}.pt"
        torch.save(model.state_dict(), os.path.join(self.save_model_dir, model_name))

        if model.name == 'Spk_Lst':
            enc_name = f"{model.name}_{epoch}_{clever_stopper_label}_spk_enc.pt"
            torch.save(model.speaker.encoder.state_dict(), os.path.join(self.save_model_dir, enc_name))
            dec_name = f"{model.name}_{epoch}_{clever_stopper_label}_spk_dec.pt"
            torch.save(model.speaker.decoder.state_dict(), os.path.join(self.save_model_dir, dec_name))

            enc_name = f"{model.name}_{epoch}_{clever_stopper_label}_lst_enc.pt"
            torch.save(model.listener.encoder.state_dict(), os.path.join(self.save_model_dir, enc_name))
            dec_name = f"{model.name}_{epoch}_{clever_stopper_label}_lst_dec.pt"
            torch.save(model.listener.decoder.state_dict(), os.path.join(self.save_model_dir, dec_name))

        else:
            pass
            # currenly only apply clever stopper during communication phase
            enc_name = f"{model.name}_{epoch if epoch else clever}_enc.pt"
            torch.save(model.encoder.state_dict(), os.path.join(self.save_model_dir, enc_name))
            dec_name = f"{model.name}_{epoch if epoch else clever}_dec.pt"
            torch.save(model.decoder.state_dict(), os.path.join(self.save_model_dir, dec_name))

        print(f'clever stopping at epoch {epoch}')

        dump_dir_mean = os.path.join(self.outputs_dir, f'{clever_stopper_label}_mean_epoch{epoch}')
        dump_dir_uttr = os.path.join(self.outputs_dir, f'{clever_stopper_label}_uttr_epoch{epoch}')
        dump_dir_msg = os.path.join(self.outputs_dir, f'{clever_stopper_label}_msg_epoch{epoch}')
        dump_dir_lstpredict = os.path.join(self.outputs_dir, f'{clever_stopper_label}_lstpred_epoch{epoch}')
