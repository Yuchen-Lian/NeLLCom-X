...
class Listener_decoder_tied(nn.Module):

    def __init__(self, shared_meaning_embedding, meaning_embedding_dim, vocab_size, meaning_len, input_size, activation_fn=nn.ReLU(), listener_type='with_embedding'):
        super(Listener_decoder_tied, self).__init__()

        self.listener_type = listener_type
        print(f'agent\'s listener_type: {self.listener_type}')
        if self.listener_type == 'with_embedding':
            # self.embedding = nn.Embedding(vocab_size, embedding_size)
            # self.embedding.weight.requires_grad = True
            self.embedding = shared_meaning_embedding
            self.embedding_dim = meaning_embedding_dim
            # shared_meaning_embedding.shape = (vocab_size, meaning_embedding_dim)
            self.meaning_len = meaning_len
            # out_features = vocab_size * meaning_len
            linear_out_feature_size = meaning_embedding_dim * meaning_len
            # self.linear = nn.Linear(input_size, out_features)
            self.linear = nn.Linear(input_size, linear_out_feature_size)
            self.activation_fn = activation_fn
            self.out_logits = nn.LogSoftmax(dim=-1)
        elif self.listener_type == 'tacl' or self.listener_type == 'relu':
            # self.embedding = nn.Embedding(vocab_size, embedding_size)
            # self.embedding.weight.requires_grad = True
            self.embedding = shared_meaning_embedding
            self.meaning_len = meaning_len
    ...
    def forward(
        ...
        batch_size = receiver_input.size(0)
        h1 = self.linear(receiver_input)
        if self.listener_type == 'with_embedding':
            h2 = torch.tensordot(h1.view(batch_size, self.meaning_len, -1), self.embedding.weight, dims=([-1], [1]))/math.sqrt(self.embedding_dim)
            output = self.out_logits(h2)
        elif self.listener_type == 'tacl':
            output = self.out_logits(h1.view(batch_size, self.meaning_len, -1))
        elif self.listener_type == 'relu':
            output = self.activation_fn(h1)
            output = self.out_logits(output.view(batch_size, self.meaning_len, -1))
        prediction = output.argmax(dim=-1)
        return output, prediction
...
class Listener_encoder_tied(EncoderRNN):
    def __init__(self, shared_word_embedding, sos_id, eos_id, pad_id, *args, **kwargs):
        super(Listener_encoder_tied, self).__init__(*args, **kwargs)
        self.sos_id = sos_id
        self.eos_id = eos_id
        self.pad_id = pad_id
        # overwrite embedding with predefined shared parameters
        self.embedding = shared_word_embedding
...
class Listener(nn.Module):
    ...
    def forward(
            ...
            labels: torch.Tensor = None, # should be meaning
        ...
        uttr_representation = uttr_representation.squeeze(dim=0)
