...
class MyDataset(Dataset):
    ...
    def __init__(self, csv_file, language, original_vocab_size=19, gen=0):
        """
        :param csv_file: Path to the csv file
        """
        self.get_language_type(language)
        # !! add generation para self.gen first in tying_embedding version (useless in this version),
        # remember to update iter_train.py when calling MyDataset(gen=new_gen)!!
        self.gen = gen
        self.aux_input_dict = {}
        self.raw_samples = pd.read_csv(csv_file, sep='\t', names=['meaning', 'utterance'], keep_default_na=False)
        self.original_vocab_size = original_vocab_size  # 10 verbs, 8 nouns, 1 mk
        self._init_dataset()
        self.max_meaning_len, self.max_uttr_len = self.get_max_len()
        self.set_aux_input_dict()
    ...
    def set_aux_input_dict(self):
        language_dict = {'fix': 0, 'fix_mk': 1, 'free_mk': 2, 'fix_op': 3, 'free_op': 4}
        lang_code = torch.tensor(language_dict[self.language])
        self.aux_input_dict.update({'language': lang_code, 'mk_idx': self.get_mk_index(), 'gen': self.gen})
    ...
    def __getitem__(self, idx):
        ...
        # gen = self.gen
        # self.aux_input_dict = {'language': lang_code, 'mk_idx': self.get_mk_index(), 'order':order, 'gen':gen}

        self.aux_input_dict.update({'order':order})

        sample = [meaning_tensor, utterance_tensor, {}, self.aux_input_dict]
    ...
    def _init_dataset(self):
        ...
        if len(counter_utterance) != (self.original_vocab_size):
            # original_vocab_size (10 nones + 8 verbs + 1 mk)
            n_unseens = self.original_vocab_size - len(counter_utterance)
            if 'mk' not in counter_utterance.keys():
                n_unseens = n_unseens - 1
                counter_utterance[f'mk'] += 1
                counter_utterance[f'mk'] -= 1

            for i in range(n_unseens):
                counter_utterance[f'unseen_{i}'] += 1
                counter_utterance[f'unseen_{i}'] -= 1
...
def my_selected_split(dataset: Dataset[T], lengths: Sequence[int],
    ...
    ************************
    ...
    if selected:
        ...
        train_samples = dataset.samples.loc[indices[:lengths[0]]]['meaning']
...
def my_split_multilang(dataset: Dataset[T], lengths: Sequence[int], overall_test_d=None,
    ...
    overall_test_indices = find_meanings_in_samples(overall_test_d, dataset)
    test_d = Subset(dataset, overall_test_indices)
    remaining_indices = [id for id in indices if id not in overall_test_indices]
    train_d_combo = Subset(dataset, remaining_indices)

    return train_d_combo, test_d


def find_meanings_in_samples(test_d, dataset):
    indices = []
    for id in test_d.indices:
        m = test_d.dataset.raw_samples.iloc[id]['meaning']
        ind = dataset.raw_samples.loc[dataset.raw_samples['meaning']==m].index.item()
        indices.append(ind)

    return indices


def my_selected_resplit(subdataset: Subset[T], lengths: Sequence[int],
                   generator: Optional[Generator] = default_generator, selected=False) -> List[Subset[T]]:
    r"""
    ************************
    If selected=True: every element appears in the train set
    ************************
    ...
    if sum(lengths) != len(subdataset):
        raise ValueError("Sum of input lengths does not equal the length of the input dataset!")

    indices = randperm(sum(lengths), generator=generator).tolist()
    train_sample_indices = [subdataset.indices[i] for i in indices]
    ...
    if selected:
        ...
        # solution 3: check current selected indices if all item appears
        train_samples = dataset.samples.loc[train_sample_indices[:lengths[0]]]['meaning']
        # train_samples = dataset.samples.loc[indices[:lengths[0]]]['meaning']
        ...
        if s == set(meaning_set):
            ...
            print(meaning_set.difference(s))
            print('!!!SELECT CALLED!!! add all items manually')
            # # solution 1: more overlap
            # flgs = [0]*len(meaning_list)
            # selected_idx = []
            # selected_samples = []
    ...
    return [Subset(dataset, train_sample_indices[offset - length : offset]) for offset, length in zip(_accumulate(lengths), lengths)]
